...
public sealed class PromptBuilder : IPromptBuilder
{
    ...
    public string BuildTurnPrompt(JsonElement context, string playerMessage)
    {
        ...
        builder.AppendLine("Используй запросить_бросок, когда действие игрока требует проверки и результат нельзя честно определить без кубика.");
        builder.AppendLine("Используй обновить_память только для важных фактов, текущей сцены, NPC, локаций, открытых линий и секретов мастера. Не обновляй память из-за мелких событий.");
